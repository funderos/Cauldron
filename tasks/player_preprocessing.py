# Preprocessing player (network) data
# Clustering and visualization of player data is not only based on their numerical features but also on structural (network) features that should be created an prepared beforehand to allow faster use in the tool.
...
# Mthod for storing processed player data in the specified filepath as well as a method for retrieving the right filepath when having a summoner id.
...
# Method for creating Python dictionaries containing details of a specific player and it's connections to other players.
...
# Method for retrieving player details of an alter. It will check the existance of persisted data and return it, otherwise delegates to the create player method for creating and storing it.
...
# Calculate statistics for all egos and stores them in Python dictionaries, also creates dictionaries with key-value mappings for Custom ID => PUUID and Custom ID => Matchcount which will be used in the application
results = []
internaliddict = {}
matchcount = {}
